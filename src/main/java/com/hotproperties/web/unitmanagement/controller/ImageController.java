package com.hotproperties.web.unitmanagement.controller;

import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.http.HttpStatus;
import org.springframework.web.bind.annotation.CrossOrigin;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.ResponseBody;
import org.springframework.web.bind.annotation.ResponseStatus;
import org.springframework.web.bind.annotation.RestController;

import com.hotproperties.web.unitmanagement.service.IImagesService;
import com.hotproperties.web.vo.request.ImagesRequest;
import com.hotproperties.web.vo.request.ImagesResponse;

@RestController(value="/web/image")
@CrossOrigin
public class ImageController
{

    @Autowired
    private IImagesService service;
    
    @PostMapping
    @ResponseStatus(HttpStatus.OK)
    @ResponseBody
    @RequestMapping(value="/save")
    public ImagesResponse save(@RequestBody ImagesRequest vo) {
        return service.save(vo);
    }
    
    @PostMapping
    @ResponseStatus(HttpStatus.OK)
    @ResponseBody
    @RequestMapping(value="/saveAll")
    public List<ImagesResponse> saveAll(@RequestBody List<ImagesRequest> vo) {
        return service.saveAll(vo);
    }
    
}
